...
class BinaryCondSumMAB(MAB):
    ...
    def __init__(self, means, s=-1):
        ...
        if s == -1:
            # the fixed sum is set equal to the expected sum by default
            self.sum = np.sum(means)
        else:
            self.sum = float(s)
        ...
        # S[i,j] = P(sum_{m=i}^n X_m = j) with m going from 0 to n-1
        self.S = np.zeros((self.n+1, self.n+1), dtype=float)
        self.S[self.n, 0] = 1
        for i in reversed(range(self.n)):
            # case j=m=0 first
            self.S[i, 0] = (1-self.unbiased_means[i])*self.S[i+1, 0]
            for j in range(1, self.n+1):
                # eq (2.2) in paper bu Huseby
                self.S[i, j] = self.unbiased_means[i] * \
                    (self.S[i+1, j-1]-self.S[i+1, j]) + self.S[i+1, j]

        # inv_S[i,j] = P(sum_{m=0}^{i-1} X_m = j)
        # needed to compute the biased means
        self.inv_S = np.zeros((self.n+1, self.n+1), dtype=float)
        self.inv_S[0, 0] = 1
        for i in range(1, self.n+1):
            self.inv_S[i, 0] = (1-self.unbiased_means[i-1])*self.inv_S[i-1, 0]
            for j in range(1, self.n+1):
                self.inv_S[i, j] = self.unbiased_means[i-1] * \
                    self.inv_S[i-1, j-1] + \
                    (1-self.unbiased_means[i-1])*(self.inv_S[i-1, j])
    ...
    def simu(self, steps=1):
        ...
        for t in range(steps):
            ...
            while s_part < self.sum:
                # eq (2.3) in paper by Huseby
                p = self.unbiased_means[i] * \
                    self.S[i+1, self.sum-s_part-1]/self.S[i, self.sum-s_part]
...
class BinaryExpSumMAB(MAB):
    ...
    def __init__(self, means, s=-1):
        ...
        # S[i,j] = P(sum_{m=i}^n X_m = j) with m going from 0 to n-1
        S = np.zeros((self.n+1, self.n+1), dtype=float)
        S[self.n, 0] = 1
        for i in reversed(range(self.n)):
            S[i, 0] = (1-self.init_means[i]) * \
                S[i+1, 0]                             # case j=m=0
            for j in range(1, self.n+1):
                # eq (2.2) in paper by Huseby
                S[i, j] = self.init_means[i] * \
                    (S[i+1, j-1]-S[i+1, j]) + S[i+1, j]

        # inv_S[i,j] = P(sum_{m=0}^{i-1} X_m = j)
        # needed to compute the biased means
        inv_S = np.zeros((self.n+1, self.n+1), dtype=float)
        inv_S[0, 0] = 1
        for i in range(1, self.n+1):
            inv_S[i, 0] = (1-self.init_means[i-1])*inv_S[i-1, 0]
            for j in range(1, self.n+1):
                inv_S[i, j] = self.init_means[i-1]*inv_S[i-1, j-1] + \
                    (1-self.init_means[i-1])*(inv_S[i-1, j])
